...
export const CreateInventoryDialog = forwardRef<
  HTMLDialogElement,
...
>(({ onSubmit, onClose }, ref) => {
  ...
  const avaialableItems = allItems?.filter(
    (menuItem: MenuItem) =>
      !inventoryItems?.some(
        (invItem: InventoryItem) => invItem.menu_item_id === Number(menuItem.id)
      )
  );
            ...
            <div key={list.id} className="flex justify-between items-top">
              ...
              {list.key === 'menu_item' ? (
                ...
                >
                  ...
                  {avaialableItems?.length === 0 && (
                    <option disabled>
                      All menu items already have inventory
                    </option>
                  )}
                  {avaialableItems?.map((item: MenuItem) => (
